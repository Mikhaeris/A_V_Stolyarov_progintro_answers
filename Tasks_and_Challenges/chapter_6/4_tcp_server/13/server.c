...
void process_command(int fd, char *line, int *counter)
{
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        for (int i = 0; i < FD_SETSIZE; i++) {
            ...
            ssize_t n = recv(fd, clients[i].buffer + clients[i].len, BUF_SIZE - clients[i].len - 1, 0);
            if (n <= 0) {
                close(fd);
                clients[i].used = 0;
                clients[i].len = 0;
            ...
            clients[i].len += n;
            clients[i].buffer[clients[i].len] = '\0';

            int start = 0;
            for (int j = 0; j < clients[i].len; j++) {
                if (clients[i].buffer[j] == '\n') {
                    clients[i].buffer[j] = '\0';
                    process_command(fd, clients[i].buffer + start, &counter);
                    start = j + 1;
                }
            }
