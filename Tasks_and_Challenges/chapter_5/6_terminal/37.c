...
static const char *WHOAMI = "daemon 5.37";
...
static void write_status(int fd, time_t start, const char *reason)
{
    char line[256];
...
int main(void)
{
    ...
    for (;;) {
        ...
        if (got_usr1) {
            got_usr1 = 0;
            write_status(fd, start, "SIGUSR1");
        }
        if (got_alarm) {
            got_alarm = 0;
            write_status(fd, start, "timer");
        }
    }
}
