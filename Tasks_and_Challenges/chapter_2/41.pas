...
procedure LLFindMaxCount(var list: LinkedList; var max_count: longint);
var
    temp: nodeptr;
begin
    max_count := 0;
...
begin
    ...
    LLFindMaxCount(list, max_count);

    writeln(#10'Answer:');
    temp := list.head;
    while temp <> nil do
    begin
        if temp^.count = max_count then
            write(temp^.data, ' ');
        temp := temp^.next;
    end;
    writeln;
