...
static void vec_words_print(vec *vec_words)
{
    ...
    for (size_t i = 0; i < vec_words->size; i++) {
        vec *vec_chars = *(vec **)vec_at(vec_words, i);
        if (vec_chars->size == max_size) {
            fwrite(vec_at(vec_chars, 0), vec_chars->type_size, vec_chars->size, stdout);
            if (count > 1) {
                printf(" ");
            }
            count -= 1;
